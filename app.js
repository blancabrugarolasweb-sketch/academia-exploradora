...
const STORAGE_KEY = "martina_exploradora_state_v09";
...
function loadState() {
  ...
  return { stars: 0, unlockedCards: [], cycleIndex: 0, completedExercises: 0, earnedBadges: [] };
}
...
function renderHome() {
  const worlds = GAME_DATA.worlds.map(w => `
    <button class="world-card ${w.status === "locked" ? "locked" : "active"}" ${w.status === "active" ? `onclick="go('botanica')"` : ""} style="background-image:url('${w.homeImage || ''}')">
      <div class="world-card-overlay">
        <div class="world-name">${w.name}</div>
        <p>${w.description}</p>
        <span class="badge">${w.status === "active" ? "Disponible" : "Próximamente"}</span>
      </div>
    </button>`).join("");
  screen("home-screen", `
...
function openLesson(id) { currentLesson = botanyWorld().lessons.find(l => l.id === id); feedback = null; route = "lesson"; render(); }
...
function renderCardIntro() {
  const c = currentCard;
  screen("card-screen", `<section class="panel"><div class="card-layout"><div class="card-preview"><div class="mystery">?</div></div><div><div class="step">Pista real</div><h2>Cromo oculto</h2><p>La Academia ha encontrado una planta misteriosa en el jardín.</p><div class="notebook"><h3>Tu pista</h3><p>${c.clue}</p></div><div class="btn-row" style="justify-content:flex-start"><button class="btn btn-primary" onclick="startCardPuzzle()">Hacer la palabra</button><button class="btn btn-secondary" onclick="go('album')">Volver al álbum</button></div></div></div></section>`);
}
function startCardPuzzle() { puzzle = makePuzzle(currentCard.targetWord); feedback = null; route = "cardPuzzle"; render(); }
...
function checkPuzzle() { const guess = puzzle.placed.map(i => i.char).join(""); if (guess.length < puzzle.target.length) { feedback = "Faltan letras."; return render(); } if (guess === puzzle.target) unlockCard(currentCard.id); else { feedback = "Casi. Prueba a cambiar el orden."; render(); } }
...
function unlockCard(id) { if (!state.unlockedCards.includes(id)) { state.unlockedCards.push(id); state.stars += 5; saveState(); } currentCard = botanyWorld().cards.find(c => c.id === id); route = "unlock"; render(); }
...
function createOperationBanks() {
  ...
  GAME_DATA.objectBank.forEach(obj => {
    for (let a = 1; a <= 7; a++) for (let b = 1; b <= 5; b++) add.push({ title: "Suma botánica", prompt: `${a} ${obj.plural} + ${b} ${obj.plural} = ?`, visual: `${obj.emoji.repeat(a)} + ${obj.emoji.repeat(b)}`, answer: a+b, options: [a+b, a+b+1, Math.max(1, a+b-1)] });
    for (let a = 3; a <= 9; a++) for (let b = 1; b < a && b <= 5; b++) subtract.push({ title: "Resta botánica", prompt: `${a} ${obj.plural} - ${b} ${obj.plural} = ?`, visual: `${obj.emoji.repeat(a)} → se van ${b}`, answer: a-b, options: [a-b, a-b+1, Math.max(1, a-b-1)] });
  });
  return { add, subtract };
}
...
function makeExercise(type) {
  ...
  if (type === "count") { const obj = pick(GAME_DATA.objectBank); const n = Math.floor(Math.random() * 8) + 2; return { type, title: "Cuenta y completa", prompt: `¿Cuántas ${obj.plural} hay?`, visual: obj.emoji.repeat(n), answer: n, options: shuffle([n, n+1, Math.max(1, n-1)]) }; }
  if (type === "syllables") { const w = pick(GAME_DATA.wordBank); return { type, title: "Cuenta sílabas", prompt: `¿Cuántas sílabas tiene ${w.word.toLowerCase()}?`, visual: w.word, answer: w.syllables, options: shuffle([w.syllables, w.syllables+1, Math.max(1, w.syllables-1)]) }; }
  if (type === "sentence") { const s = pick(GAME_DATA.sentenceBank); return { type, title: "Cuenta palabras", prompt: "¿Cuántas palabras tiene esta frase?", visual: s.text, answer: s.words, options: shuffle([s.words, s.words+1, Math.max(1, s.words-1)]) }; }
...
function renderExercise() {
  ...
  screen("exercise-screen", `<section class="question-box"><div style="display:flex; justify-content:space-between; gap:12px; align-items:flex-start;"><div><div class="step">Juego aleatorio</div><h2>${e.title}</h2></div><button class="btn btn-secondary" onclick="go('botanica')">← Jardín</button></div><p><strong>${e.prompt}</strong></p><div class="exercise-visual">${e.visual}</div><div class="options">${options}</div>${feedback !== null ? `<div class="explanation"><strong>${String(feedback) === String(e.answer) ? "¡Muy bien!" : "Casi. La respuesta correcta era " + e.answer + "."}</strong></div><div class="btn-row"><button class="btn btn-primary" onclick="startRandomCycleExercise()">Otro juego</button><button class="btn btn-secondary" onclick="go('botanica')">Volver</button></div>` : ""}</section>`);
}
function answerExercise(option) { feedback = option; if (String(option) === String(currentExercise.answer)) { state.stars += 1; state.completedExercises += 1; saveState(); } render(); }
